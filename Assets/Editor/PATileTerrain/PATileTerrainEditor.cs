...
public class PATileTerrainEditor : Editor 
{
	...
	private string[] brushNamesPaint = {"1x1", "3x3", "5x5"};
	private int[] brushSizesPaint = {1, 3, 5};
	...
	public void OnSceneGUI() 
    {
		...
		if (currentWindow)
		{
			...
			case EventType.MouseDown: 
				...
				{
					startAction = true;
					if (menuToolbar == MENU_HEIGHTS && !current.alt)
					{	
						SetHeights((current.shift == true));
                    }
                    else if (menuToolbar == MENU_PAINT && !current.alt)
					{
						Paint();
					}
			...
			case EventType.MouseDrag:
				if (startAction && menuToolbar == MENU_HEIGHTS && !current.alt)
				{
					SetHeights((current.shift == true));
                }
                else if (startAction && menuToolbar == MENU_PAINT && !current.alt)
				{
					Paint();
				}
	...
	public void OnEnable()
	{
		...
		{
			EditorUtility.SetSelectedWireframeHidden(c.gameObject.GetComponent<Renderer>(), true); 
		}
	...
	public void OnInit()
	{
		menuSettings = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/settings.png", typeof(Texture)) as Texture;
		menuHeights = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/heights_colors.png", typeof(Texture)) as Texture;
		menuPaint = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/paint.png", typeof(Texture)) as Texture;
		menuTile = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/tile.png", typeof(Texture)) as Texture;
		...
		whiteTex = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/white.png", typeof(Texture2D)) as Texture2D;
		blackTex = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/black.png", typeof(Texture2D)) as Texture2D;
			
		for (int i = 0; i < 14; ++i)	
			transitionsBits[i] = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Textures/bits_" + i + ".png", typeof(Texture2D)) as Texture2D;
		...
		do 
		{
			brush = AssetDatabase.LoadAssetAtPath("Assets/PATileTerrain/Brushes/patileterrain_brush" + brushCount + ".png", typeof(Texture2D)) as Texture2D;
	...
	{		
        ...
        int shuijingLayer = LayerMask.NameToLayer("Shuijing");
        int layermask = 1 << cameraMoveAreaLayer | 1 << shuijingLayer;
        layermask = ~layermask;
	...
	{		
        //if (brushSizePaint == 3) 
        //    tileTerrain.PaintTile3x3(GetTile(), tilesetBrushe);
        //else if(brushSizePaint == 5)
        //    tileTerrain.PaintTile5x5(GetTile(), tilesetBrushe);
        //else 
        //    tileTerrain.PaintTile1x1(GetTile(), tilesetBrushe);
        tileTerrain.PaintTile(GetTile(), tilesetBrushe, brushSizePaint); 	
	}
    ...
    {		
		...
		{	
            ...
            if (GUILayout.Button("Create Terrain From File"))
            {
		...
		{		
			...
			case MENU_TILE:
					...
					} else
					{
                        ...
                        GUILayout.EndHorizontal();
                        GUILayout.BeginHorizontal();
                        GUILayout.Label("QtrTile_LT : ", EditorStyles.boldLabel, GUILayout.MaxWidth(96));
                        GUILayout.Label(selectedTile.qtrTiles[1].ToString() + " (read only)");
                        GUILayout.EndHorizontal();
                        GUILayout.BeginHorizontal();
                        GUILayout.Label("QtrTile_RT : ", EditorStyles.boldLabel, GUILayout.MaxWidth(96));
                        GUILayout.Label(selectedTile.qtrTiles[2].ToString() + " (read only)");
                        GUILayout.EndHorizontal();
                        GUILayout.BeginHorizontal();
                        GUILayout.Label("QtrTile_RB : ", EditorStyles.boldLabel, GUILayout.MaxWidth(96));
                        GUILayout.Label(selectedTile.qtrTiles[3].ToString() + " (read only)");
                        GUILayout.EndHorizontal();
			...
			//if (EditorUtility.DisplayDialog  ("Confirm Destroy" , "Do you really want to destroy the terrain?", "Yes", "No"))
			{
                ...
                //}
                FileStaticAPI.DeleteFolder("PATileTerrain_" + tileTerrain.settings.name);
               // "Assets", 
				AssetDatabase.StopAssetEditing();
	...
	public void SaveTileset(string path)
	{
        ...
        Debug.Log(tileTerrain.settings.tilesetMaterial.name);
	...
	public void OpenTileset(string path)
	{
		...
		if (version == 1)
		{
			...
			tileTerrain.settings.tilesetMaterial = AssetDatabase.LoadAssetAtPath(f.ReadLine(), typeof(Material)) as Material;
